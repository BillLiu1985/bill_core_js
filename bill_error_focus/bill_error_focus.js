(function(jQuery,bill_core){
	...
	jQuery.bill_error_focus={
		...
		'v2_attach':function(jquery_expression){
			if(bill_core.global_typeof(jquery_expression)!=='string'){
				bill_core.debug_console('jquery_expression error','error')
				return;
			}
			this.v2_deattach(jquery_expression);
			if(jQuery.bill_error_focus.action==='add_class'){
				jQuery(jquery_expression).filter(this.input_jquery_expression).each(
					function(){
						if(jQuery(this).attr('validate_fail_message')!==''){
							jQuery(this).addClass("input_validation-failed");
						}
		...
		'v2_deattach':function(jquery_expression){
			if(bill_core.global_typeof(jquery_expression)!=='string'){
				bill_core.debug_console('jquery_expression error','error')
				return;
			}
			if(jQuery.bill_error_focus.action==='add_class'){
				jQuery(jquery_expression).filter(this.input_jquery_expression).removeClass("input_validation-failed");
			}
		},
